...
class DSNode (Node):
    ...
    def __init__(self, host, port, id_num, n, delta, id=None, callback=None, max_connections=5):
    ...
    def node_message(self, node, data):
        ...
        if num_sign >= round_num+1:
            ...
            print("Message received in round " + str(round_num) + ": " + messagebody)
            self.send_to_nodes(Message(messagebody, isBroadcast, num), exclude=[node])
    ...
    def start_the_protocol(self, num):
        self.pool.add(f"Message #{num}")
        self.send_to_nodes(Message(f"Message #{num}", False, 1<<self.id_num))